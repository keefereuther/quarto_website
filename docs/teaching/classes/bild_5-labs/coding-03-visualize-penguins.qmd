...
Welcome back. Everything on this page runs real R in your browser — same setup as Coding Activity 2. You already plotted with base R's `hist()`; today you will **build figures in layers** with ggplot2.
...
Start a figure with `ggplot()`, map `body_mass_g` to the x-axis, and add `geom_histogram()`. Use the `penguins` dataset directly — an upgrade from Coding Activity 2's `hist()`. Add a title with `labs(title = "...")`.
...
Boxplots compare a **continuous variable** across **groups**. On your graded assignment you will add `reorder()` to sort groups by elevation after you join that metadata in Coding Activity 4.
:::
:::
...
## 6. Recap — ggplot functions you practiced
...
You practiced the layering here. Coding Activity 4 covers the wrangling steps — filtering, reshaping, and joining — that prepare the table for those plots.
